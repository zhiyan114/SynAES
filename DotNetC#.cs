...
class SynAES {
	...
	public string decrypt(string data, string iv) {
		...
		byte[] Ciphertxt = new byte[arrdata.Length-12];
		Buffer.BlockCopy(arrdata,0,Ciphertxt,0,arrdata.Length-12);
		byte[] Tag = new byte[12];
		Buffer.BlockCopy(arrdata, arrdata.Length-12,Tag,0,12);
		byte[] Plaintxt = new byte[arrdata.Length-12];
