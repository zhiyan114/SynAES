...
export default class SynAES {
  ...
  encrypt(data: string, iv: string): string {
    ...
    return Buffer.concat([encryptedBuffer, cipher.getAuthTag()]).toString(
      'base64',
    );
  }
  decrypt(data: string, iv: string): string {
    const byteData = Buffer.from(data, 'base64');
    const datalen = Buffer.byteLength(byteData);
    const decipher = createDecipheriv(this.gcmalg, this.key, iv);
    decipher.setAuthTag(byteData.subarray(datalen - 16, datalen));
    return Buffer.concat([
      decipher.update(byteData.subarray(0, datalen - 16)),
      decipher.final(),
    ]).toString();
  }
}
